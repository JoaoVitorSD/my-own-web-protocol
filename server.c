...
void init_peer_connection(char *peerPort, server_t *server)
{
    ...
    if (0 != connect(s, addr, sizeof(storage)))
    {
        ...
        server->peer_socket = init_server(peerPort, &storage);
        return;
    }
    ...
    server->current_peer_id = atoi(response.payload);
...
void handle_peer_req(server_t *server, int client_sock, char *clientInfo)
{
    ...
    if (action == REQ_CONNPEER)
    {
        ...
        if (server->current_peer_id != -1)
        {
            return_response(client_sock, ERROR, "01");
            return;
        }
        server->current_peer_id = client_sock;
        printf("Peer connected\n");
        return_response(client_sock, 1, itoa(client_sock));
        return;
    }
    return_response(client_sock, 1, clientInfo);
}
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        int peerSock = accept_conncetion(server->peer_socket, peer_addrstr, peer_buffer);
